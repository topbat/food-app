...
## [2026-06-12] 内容数据扩充：菜谱翻倍、社区填充、图片素材与菜品一一对应

- 新增：本地图片素材库 `frontend/public/images/`（共 32 张）—— 19 张菜品成品实拍图（取自 Wikimedia Commons，逐张人工核对与菜名一致）、6 张备/洗/切/煮通用过程图、7 个用户头像 SVG；菜谱封面与步骤图全部由 picsum 随机图换为对应素材，装盘（PLATE）步骤直接复用该菜成品图
- 新增：recipe-service 种子菜谱由 10 道扩至 **19 道**（红烧肉/番茄炒蛋/鱼香肉丝/回锅肉/酸菜鱼/扬州炒饭/清蒸鲈鱼/可乐鸡翅/蒜蓉西兰花），配套步骤 136 条（含计时/火候）、食材关联 144 条、食材库扩至 48 条（新增主食分类）、标签扩至 18 个（一人食/宴客菜）、替换规则扩至 11 条
- 新增：social-service 社区种子数据 —— 帖子 14 条（晒作品/美食日记/经验分享三类，图文与菜谱关联）、评论 48 条（含帖子评论与菜谱评论、楼中楼）、评分 18 条、打卡 19 条（含连续 7 天）、点赞记录 22 条、用户徽章 13 条
- 新增：user-service DevDataInitializer 创建 6 个社区演示用户（ID 2-7，密码 123456，昵称+头像），与社区帖子 user_id 对应
- 新增：search-service 热搜关键词种子 10 条及 dev 环境 `data.sql` 初始化配置，首页热搜 chips 不再空白
- 影响范围：recipe/user/search/social/frontend
